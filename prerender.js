...
async function main() {
  ...
  const index = (await readFile(join(process.cwd(), 'dist/documentation', 'index.html'))).toString();

  // Serving the static files.
  app.get('*.*', express.static(join(process.cwd(), 'dist/documentation')));
  ...
  do {
    ...
    const file = join(process.cwd(), 'dist/documentation', (p || 'index') + '.html');
